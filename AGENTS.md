...
- **技能同步**：Hermes 技能库 `~/.agents/skills/coding/rail-cli` 是本仓库的 git 克隆（remote 指向 GitHub）。仓库侧改完 commit+push 后，技能侧 `cd ~/.agents/skills/coding/rail-cli && git pull` 同步；技能侧直接改 SKILL.md 则 commit+push 后仓库侧 `git pull`。两处是同一仓库的独立 clone，改动必须通过 push/pull 流转，不要只改文件
